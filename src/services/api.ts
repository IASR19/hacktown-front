...
import { Venue, Activity, SlotTemplate, DaySlotActivity, WeekDay, EventConfig } from '@/types/hacktown';
...
export const eventConfigService = {
  ...
  async updateConfig(selectedDays: WeekDay[]): Promise<EventConfig> {
    return apiClient.put<EventConfig>('/event-config', { selectedDays });
  },
};
...
export const slotTemplatesService = {
  ...
  async bulkUpsert(slotTemplates: Omit<SlotTemplate, 'id'>[]): Promise<SlotTemplate[]> {
    return apiClient.post<SlotTemplate[]>('/slot-templates/bulk', slotTemplates);
  },

  async update(id: string, slotTemplate: Omit<SlotTemplate, 'id'>): Promise<SlotTemplate> {
...
export const daySlotActivitiesService = {
  ...
  async getBySlotAndDay(slotTemplateId: string, day: WeekDay): Promise<DaySlotActivity | null> {
  ...
  async create(data: { slotTemplateId: string; day: WeekDay; activityId: string }): Promise<DaySlotActivity> {
    return apiClient.post<DaySlotActivity>('/day-slot-activities', data);
  },

  async bulkUpsert(daySlotActivities: Array<{ slotTemplateId: string; day: WeekDay; activityId: string }>): Promise<DaySlotActivity[]> {
    return apiClient.post<DaySlotActivity[]>('/day-slot-activities/bulk', daySlotActivities);
  },

  async update(id: number, data: { activityId: string }): Promise<DaySlotActivity> {
