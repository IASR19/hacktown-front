...
import { MapPin, Clock, Mic2, Users, Sparkles, Building2, Filter, Music, Presentation, PanelTop, Wrench } from 'lucide-react';
import { DashboardCharts } from '@/components/dashboard/DashboardCharts';
import { DashboardOverview } from '@/components/dashboard/DashboardOverview';
import { WEEKDAY_SHORT_LABELS, VENUE_STRUCTURE_LABELS, WeekDay, VenueStructureType, ACTIVITY_TYPE_LABELS, ActivityType } from '@/types/hacktown';
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function Dashboard() {
  const { getVenuesWithSlots, venues, selectedDays, eventStartDate, eventEndDate, isLoading, venueDayActivities } = useHacktown();
  ...
  const [expandedDays, setExpandedDays] = useState<Record<string, Set<WeekDay>>>({});
  ...
  const venuesByActivityType = useMemo(() => {
    const groupedByType: Record<ActivityType, Array<typeof venues[number] & { days: WeekDay[] }>> = {
      musica: [],
      palestra: [],
      painel: [],
      workshop: [],
    ...
    venues.forEach((venue) => {
      ...
      if (filterStructure !== 'all' && venue.structureType !== filterStructure) return;
      ...
      selectedDays.forEach((day) => {
        ...
        // Add day to venue
        const existingVenue = groupedByType[activityType].find(v => v.id === venue.id);
        if (existingVenue) {
    ...
    Object.keys(groupedByType).forEach((key) => {
      ...
      groupedByType[activityType] = groupedByType[activityType].filter(v => v.days.length > 0);
    });

    return groupedByType;
  }, [venues, selectedDays, venueDayActivities, filterDay, filterNucleo, filterStructure]);
          ...
          {selectedDays.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Venues por Tipo de Atividade</h3>
              </div>
              
              {/* Compact Cards */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {(Object.keys(ACTIVITY_TYPE_LABELS) as ActivityType[]).map((activityType) => {
                      ...
                      <DialogHeader>
                        <DialogTitle className="flex items-center gap-3 text-2xl gradient-text">
                          {(() => {
                            ...
                            const color = getActivityColor(selectedActivityType);
                            return <Icon className={`h-6 w-6 ${color}`} />;
                          })()}
                          {ACTIVITY_TYPE_LABELS[selectedActivityType]}
                          <Badge variant="outline" className="ml-2">
                            {venuesByActivityType[selectedActivityType].length} venue(s)
                          </Badge>
                        </DialogTitle>
                      </DialogHeader>
                      
                      <div className="space-y-4 mt-4">
                        {venuesByActivityType[selectedActivityType].map((venue) => {
                          const venueWithSlots = venuesWithSlots.find(v => v.id === venue.id);
                          const sortedDays = [...venue.days].sort((a, b) => 
                            selectedDays.indexOf(a) - selectedDays.indexOf(b)
                          );
                            ...
                            >
                              ...
                              <div className="flex items-start justify-between mb-4">
                                ...
                                <div className="flex gap-2">
                                  <Badge variant="outline" className="border-hacktown-cyan/30">
                                    <Users className="h-3 w-3 mr-1" />
                                    {venue.capacity}
                                  </Badge>
                                  {venue.nucleo && (
                                    <Badge variant="outline" className="border-hacktown-pink/30">
                                      {venue.nucleo}
                                    </Badge>
                                  )}
                                </div>
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {sortedDays.map((day) => {
                                    const daySlots = venueWithSlots?.slots.filter(s => s.day === day) || [];
                                    const dayWithDate = getDayWithDate(day);
                                    const isExpanded = expandedDays[venue.id]?.has(day);
                                    ...
                                    return (
                                      <div key={day} className="flex-1 min-w-[200px]">
                                        <Badge 
                                          ...
                                          onClick={() => toggleDay(venue.id, day)}
                                        >
